...
md"""
# ES222: Inverse Problems — Discrete Grid Demo
...
# ╔═╡ 7d2a2f62-45f9-4c33-a458-403b2a6dc2d8
@bind v Slider(4.5:0.1:9.5, default=7.0, show_value=true)

# ╔═╡ 9c3af1ad-b9ff-4e28-9c42-68a0b5635f3a
@bind noise_halfwidth Slider(0.0:0.01:0.80, default=0.12, show_value=true)

# ╔═╡ 4d3ee2ff-82d9-4f8b-98d3-10b0f3c1f610
@bind n_per_level Slider(1:1:20, default=8, show_value=true)

# ╔═╡ 58e07a8a-63e2-49be-9e83-3404f7ad9c27
@bind x_low Slider(0.0:0.1:3.0, default=1.2, show_value=true)

# ╔═╡ 06e5a6d2-d6b3-4b8b-86a5-8b4b6f0dd6f2
@bind x_high Slider(0.2:0.1:4.0, default=2.4, show_value=true)

# ╔═╡ 2b8a8f41-1950-4d89-9f95-3b07f4a7b1ff
@bind base_seed Slider(1:1:999, default=42, show_value=true)

# ╔═╡ 1d77f1ad-1934-41d6-93f8-4f14b6b1b7ef
...
begin
	...
	# Generate points (uniform noise)
	points = map(1:3) do k
		x = x_base[k] .+ (2 .* rand(n_per_level) .- 1) .* noise_halfwidth
		y = y_levels[k] .+ (rand(n_per_level) .- 0.5) .* 0.25
		(; x, y)
	end
	...
	(; z_levels, labels, y_levels, x_base, points, counts, posterior)
end
...
TikzPictures = "37f6aa50-8035-52a5-8ae6-6b299c0b7e43"
...
manifest_format = "2.0"
project_hash = "496a56a212bbacf5492e395506794018df7f9ac2"
