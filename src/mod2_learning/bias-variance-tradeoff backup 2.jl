...
- Optimizer: **Adam**
- Input → hidden connections: **fixed** with uniformly spaced joints over x
...
begin
    ...
    struct FixedJointFeatures
        joints::Vector{Float32}
        scale::Float32
    end

    function (layer::FixedJointFeatures)(x)
        xvec = vec(x)
        return tanh.((reshape(xvec, 1, :) .- reshape(layer.joints, :, 1)) ./ layer.scale)
    end

    function make_model(hidden_units::Int)
        joints = collect(range(xmin, xmax, length=hidden_units))
        dx = hidden_units > 1 ? (xmax - xmin) / (hidden_units - 1) : (xmax - xmin)
        feature_layer = FixedJointFeatures(Float32.(joints), Float32(max(dx, 1f-3)))
...
manifest_format = "2.0"
project_hash = "d0d3e80770da279a6ef35f025dbb3b869365890b"
