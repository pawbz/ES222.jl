...
begin
    ...
    Flux.@layer HomoModel

    (m::HomoModel)(x) = m.net(x)
    ...
    Flux.@layer HeteroModel

    function (m::HeteroModel)(x)
        mu = m.net_mu(x)
        log_sigma = m.net_logsigma(x)
        return mu, log_sigma
    end
    ...
    function init_model(kind::String, seed::Int)
        ...
        if kind == "Homoscedastic"
            net = Flux.f32(Chain(Dense(1, 64, relu), Dense(64, 1)))
            return HomoModel(net, -1.0f0)
        else
            net_mu = Flux.f32(Chain(Dense(1, 64, relu), Dense(64, 1)))
            net_logsigma = Flux.f32(Chain(Dense(1, 64, relu), Dense(64, 1)))
            return HeteroModel(net_mu, net_logsigma)
        end
    end
...
trained = @use_memo([params.epochs, params.lr, params.kind, params.reinit, params.resample, params.ntrain, params.noise, params.outliers, params.seed]) do
    ...
    losses = Float32[]
    snapshots = Vector{Any}(undef, params.epochs + 1)
    snapshots[1] = deepcopy(model)
    for epoch in 1:params.epochs
        Flux.train!(model, data.loader, opt) do m, x, y
            nll(m, x, y)
        end
        push!(losses, nll(model, data.xtrain, data.ytrain))
        snapshots[epoch + 1] = deepcopy(model)
    end

    (; model, losses, snapshots)
end
...
# ╔═╡ 1c10d1bc-70a8-4a02-93a4-6c1d59ec3a3a
md"""## Epoch-wise p(y|x)"""

# ╔═╡ 0a0c1a7b-5c3c-4e83-9d09-7c0f4b2f6a7c
function gui_view_controls()
    return PlutoUI.combine() do Child
        inputs = [
            md"""View epoch: $(Child("epoch_view", Slider(0:1:params.epochs, default=params.epochs, show_value=true)))""",
            md"""x location: $(Child("x_view", Slider(xmin:0.05:xmax, default=0.0, show_value=true)))"""
        ]
        md"""$(inputs)"""
    end
end

# ╔═╡ 5d3c8c3f-8b0f-46a4-8c08-7d4d9a92fb2c
view_controls = @bind view_params gui_view_controls()

# ╔═╡ 6c6c8f03-bb29-4c5f-9a86-4d5a6c2a482c
let
    epoch_idx = clamp(view_params.epoch_view + 1, 1, length(trained.snapshots))
    model_view = trained.snapshots[epoch_idx]

    x0 = Float32(view_params.x_view)
    x0m = reshape([x0], 1, 1)
    mu0, s0 = predict_mu_sigma(model_view, x0m)
    mu0 = Float64(mu0[1])
    s0 = Float64(s0[1])
    ...
    traces = [
        scatter(x=vec(data.xtrain), y=vec(data.ytrain), mode="markers", name="data", marker=attr(color="rgba(185,113,80,0.25)", size=6)),
        scatter(x=xplot, y=vec(predict_mu_sigma(model_view, reshape(Float32.(xplot), 1, :))[1]), mode="lines", name="model mean", line=attr(color="rgba(36,122,115,1)", width=2)),
        scatter(x=Float64(x0) .+ scale .* pdf_vals, y=ygrid, mode="lines", name="p(y|x)"),
        scatter(x=[Float64(x0)], y=[mu0], mode="markers", marker=attr(color="rgba(36,122,115,1)", size=8), showlegend=false)
    ]

    title_text = "p(y|x) at epoch $(view_params.epoch_view)"
    plot(traces, Layout(width=650, height=420, title=title_text, xaxis=attr(title="x"), yaxis=attr(title="y")))
end
...
manifest_format = "2.0"
project_hash = "ea8e31b5bce44d66ba2ca3829f530fe420549670"
