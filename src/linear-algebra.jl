...
# ╔═╡ 3e72b5ba-7e08-4ddc-89fa-211aff48ef90
@bind num_basis Slider(1:10, show_value=true)
...
# ╔═╡ 05fd56f6-3629-441a-a44c-5bc0e77c36e4
...
resample_button = @bind resample1 CounterButton("Resample")

# ╔═╡ 2b856ffb-56c9-4fe8-8e3a-5360d5059239
resample_button

# ╔═╡ bda26e18-6ec8-4b2d-abc2-0fd17310f96c
resample1; v1 = randn(3)

# ╔═╡ 5842ae5e-e575-47a1-8501-9b74967fb769
resample1; v2 = randn(3)
...
function myquiver(vectors, names=fill("", length(vectors)), colors=fill("black", length(vectors)), title="")
    ...
    layout = Layout(
        ...
        scene=attr(
            uirevision=1,
            aspectmode="manual", aspectratio=attr(x=1, y=1, z=1),
            xaxis=attr(title="X-axis", nticks=4, range=[-5, 5]),
            yaxis=attr(title="Y-axis", nticks=4, range=[-5, 5]),
            zaxis=attr(title="Z-axis", nticks=4, range=[-5, 5])
        )
    )

    # Create both lines and cones for each vector
    plots = Vector{PlotlyBase.AbstractTrace}()
    for (vec, name, color) in zip(vectors, names, colors)
        # Line from the origin to the vector tip
        push!(plots, scatter3d(
            x=[0, vec[1]],
            y=[0, vec[2]],
            z=[0, vec[3]],
            mode="lines",
        ...
        # Arrow (cone) at the tip of the vector
        push!(plots, cone(
            x=[vec[1]], y=[vec[2]], z=[vec[3]],  # Base of the cone
            u=[vec[1]], v=[vec[2]], w=[vec[3]],  # Direction of the arrow
            sizemode="absolute",
            sizeref=0.6,  # Adjust arrow size
            colorscale=[[0, color], [1, color]],  # Solid color for the arrow
            showscale=false,
            anchor="tail",  # Anchor the cone tip to the vector tip
            name=""
        ))
    end

    # Combine all plots and return
    return plot(plots, layout)
end
...
#     # Create the cone plot for each vector
#     cones = [
#         cone(
#             x=[0], y=[0], z=[0],  # Base of the arrow
#             u=[vec[1]], v=[vec[2]], w=[vec[3]],  # Direction of the arrow
#             sizemode="absolute",
...
#     # Combine all cone plots and return
#     return plot(cones, layout)
# end

# ╔═╡ d3434aab-20ed-469f-b11e-60a97541b251
myquiver([v1, v2, projection(v1, v2)], ["G¹", "m", "projection of m on G¹"], ["black", "blue", "red"], "Inner Product")
...
manifest_format = "2.0"
project_hash = "bd95814b2427d1f93fcc48e809a824d9c36c0bc4"
...
git-tree-sha1 = "e3b1981b15022905b424e4181cc7a36909e3161a"
